...
export default function PlaylistDetailPage() {
  ...
  /** Rebuild the Up Next queue from an updated song list, keeping the currently playing song in place */
  const syncQueue = useCallback(
    (updatedSongs: PlaylistSong[]) => {
      if (!currentSong) return
      const currentIndex = updatedSongs.findIndex((s) => s.song_data.trackId === currentSong.trackId)
      if (currentIndex === -1) return // current song isn't from this playlist
      setQueue(updatedSongs.slice(currentIndex + 1).map((s) => s.song_data))
    },
  ...
  async function handleRemoveSong(songId: string) {
    const { error } = await removeSong(songId)
    if (error) {
      return
    }
  ...
  async function handleMoveSong(songId: string, newPosition: number) {
    const { error } = await moveSong(playlistId, songId, newPosition)
    if (error) {
      return
    }
    const { data } = await fetchSongs(playlistId)
  ...
  function handlePlayFrom(index: number) {
    ...
    if (!song) return
    play(song.song_data)
    setQueue(songs.slice(index + 1).map((s) => s.song_data))
  }
        ...
        {!isLoading && songs.length > 0 && (
          <ul className="divide-y divide-gray-200 dark:divide-gray-800">
            {songs.map((song, index) => {
              const isActive = currentSong?.trackId === song.song_data.trackId
              ...
              return (
                <li key={song.id} className="flex items-center gap-4 py-3">
                  <button
                    ...
                    aria-label={`Play ${song.song_data.trackName}`}
                  >
                    <span
                      className={[
                        "w-6 text-right text-sm tabular-nums",
                        isActive ? "font-bold text-violet-600 dark:text-violet-400" : "text-gray-400",
                      ].join(" ")}
                    >
                      {isActive ? "\u25B6" : index + 1}
                    </span>
                    ...
                    <div className="min-w-0 flex-1 text-left">
                      <p
                        className={[
                          ...
                          isActive ? "text-violet-600 dark:text-violet-400" : "text-gray-900 dark:text-gray-100",
                        ].join(" ")}
                      >
