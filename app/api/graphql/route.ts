...
const GRAPHQL_API_URL = env.GRAPHQL_API_URL ?? "http://localhost:9000/graphql"
const GRAPHQL_INTERNAL_SECRET = env.GRAPHQL_INTERNAL_SECRET
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ error: "Bad response from upstream server" }, { status: 502 })
  }
}
